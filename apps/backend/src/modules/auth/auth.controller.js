...
exports.register = async (req, res) => {
  try {
    ...
    const normalizedAlternatePhone = normalizePhoneNumber(alternatePhone);
    ...
    if (alternatePhone && !PHONE_REGEX.test(normalizedAlternatePhone)) {
      return res.status(400).json({
        success: false,
        message: 'Validation failed',
        errors: [{ message: 'Please provide a valid alternate phone number' }],
      });
    }
